...
class PNG {
    ...
    private:
        ...
        void read_IHDR(){
            ...
            width_total_size = width * bytes_per_pixel + 1;
        }
        ...
        unsigned short int Recon_a(const unsigned int heightPosition, const unsigned int widthPosition) {
            if (widthPosition >= (bytes_per_pixel * ((bit_depth > 8) ? 2 : 1))) {
                return idat_chunk_data[(heightPosition * width_total_size) + widthPosition - bytes_per_pixel];
            } else {
                return 0;
            }
        }
        ...
        void read_IDAT(){
            ...
            vector<unsigned char> buffer(height*width*bit_depth/8);

            unsigned int index = 0;
            unsigned short int filter_method;
            ...
            for (unsigned int h = 0; h < 1; h++) {
                ...
                for (unsigned int w = 0; w < width; w++) {
                    unsigned short int pixel_x = idat_chunk_data[index];
...
int main() {
    string nome = "01-crop.png";
    //string nome = "pngsuite-main/png/basn2c08.png";    
    //getline(cin, nome);
    PNG imagem = PNG(nome);
